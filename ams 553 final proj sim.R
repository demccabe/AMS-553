...
#        cust_order = i_du + floor((j_du - i_du + 1) * runif(1,0,1))
        cust_order = max(0,floor(rnorm(1,20,2)))
      ...
      if (Cur_inv_level < little_s && order_status==0) {
        ...
        foo_backlog_mean = 0
        for (i in 1:100) {
#          cnew[i] = max(0,alpha*(mu - c) + rnorm(1,0,20))
          cnew[i] = max(0,mu + rnorm(1,0,rnorm(1,20,1)))
          foo_cust[i] = 1
#          foo_ord[i] = i_du + floor((j_du - i_du + 1) * runif(1,0,1))
          foo_ord[i] = max(0,floor(rnorm(1,20,2)))
      ...
      if (Cur_inv_level < 0) {
        ...
        Penalty_cost[t+1] = 0
      c = max(0, alpha*(mu - c) + rnorm(1,0,20))
      print(c)
